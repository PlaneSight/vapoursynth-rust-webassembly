...
browser_meson_args=()
browser_threaded_requested=0
case "${VS_BROWSER_THREADED:-}" in
    1|true)
        browser_meson_args+=('-Dbrowser_threaded=true')
        browser_threaded_requested=1
        ;;
    0|false)
        browser_meson_args+=('-Dbrowser_threaded=false')
        ;;
    '')
        browser_meson_args+=('-Dbrowser_threaded=false')
        ;;
    *)
        echo "error: VS_BROWSER_THREADED must be 0/1 or false/true" >&2
        exit 64
        ;;
esac
if [[ -n "${VS_BROWSER_THREAD_POOL_SIZE:-}" ]]; then
    browser_meson_args+=("-Dbrowser_thread_pool_size=${VS_BROWSER_THREAD_POOL_SIZE}")
fi

meson_setup_args=(
    --cross-file toolchains/emscripten.ini
    --buildtype debug
    "${browser_meson_args[@]}"
)
if [[ -d build/emscripten/meson-private ]]; then
    uv run --locked meson setup --reconfigure build/emscripten . "${meson_setup_args[@]}"
else
    uv run --locked meson setup build/emscripten . "${meson_setup_args[@]}"
fi
...
if (( browser_threaded_requested )); then
    compiled_threading_mode="threaded"
else
    compiled_threading_mode="single-thread"
fi
printf '%s\n' \
    '// Generated by tools/build-browser.sh; do not edit.' \
    "export const COMPILED_THREADING_MODE = \"$compiled_threading_mode\";" \
    > "$runtime_dir/vapoursynth/threading.mjs"
...
if (( browser_threaded_requested )); then
    # Older Emscripten releases emit a companion worker module; newer releases
    # inline its runtime, so copy it when present without requiring one.
    module_stem="${module_js%.js}"
    for worker_asset in "$module_stem.worker.js" "$module_stem.worker.mjs"; do
        if [[ -f "$worker_asset" ]]; then
            cp "$worker_asset" "$runtime_dir/$(basename "$worker_asset")"
        fi
    done
fi
